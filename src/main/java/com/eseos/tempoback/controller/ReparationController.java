...
public class ReparationController {
    ...
    public ResponseEntity<UserDTO> addUser(@Valid @RequestBody UserDTO userDTO, Principal principal)
        ...
        if (potentialUser.isEmpty()) {
            throw new EntityNotFoundException(ErrorUtils.USER_NOT_FOUND);
        } else if (potentialUser.get().isAdmin()) {

            strongPasswordValidator.isValid(userDTO.get_password());

            userDTO.set_password(passwordEncoder.encode(userDTO.get_password()));
            ...
            return ResponseEntity.created(ServletUriComponentsBuilder.fromCurrentRequest().path("/{email}")
                    .buildAndExpand(newUser.getEmail()).toUri()).body(modelMapper.map(newUser, UserDTO.class));
        } else {
    ...
    @ResponseStatus(HttpStatus.OK)
    public void deleteUser(@PathVariable("email") String email, Principal principal) throws RelatedDataException {
        Optional<User> potentialUser = this.userService.findUserByEmail(principal.getName());
        if (potentialUser.isEmpty()) {
            throw new EntityNotFoundException(ErrorUtils.USER_NOT_FOUND);
        } else if (potentialUser.get().isAdmin() && potentialUser.get().getEmail() != email) {
